...
router.delete("/delete", async (req, res) => {
  ...
  if (!id) {
    res
      .status(400)
      .json({ error: "missing header of id. please provide a movie id" });
    return;
  }
  ...
  try {
    ...
    if (movieToDel === null) {
      res
        .status(404)
        .send({ error: `Can not delete, no movie found with id '${id}'` });
      return;
    }
...
router.patch("/update/:id", async (req, res) => {
  ...
  if (!id || !req.body) {
    return res
      .status(400)
      .json({ error: "Missing param, please provide an id" });
  }
  ...
  if (!movieToUpdate) {
    res
      .status(404)
      .json({ error: `Can not update, no movie found with id '${id}` });
    return;
  }
